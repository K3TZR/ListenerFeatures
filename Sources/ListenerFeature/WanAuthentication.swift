...
//    if _smartlinkEmail != nil, let refreshToken = _secureStore.get(account: _smartlinkEmail) {
    if _smartlinkEmail != nil, let refreshToken = refreshToken {
      // YES, can we get an Id Token from the Refresh Token?
      if let idToken = await requestIdToken(from: refreshToken, smartlinkEmail: _smartlinkEmail!), isValid(idToken) {
