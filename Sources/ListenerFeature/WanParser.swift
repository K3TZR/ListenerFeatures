...
extension WanListener {
  ...
  func parseVitaPayload(_ text: String) {
    ...
    guard let token = Property(rawValue: properties[0].key)  else {
      // log it
      log("Wan Listener: \(msg)", .warning, #function, #file, #line)
      return
    }
  ...
  private func parseApplication(_ properties: KeyValuesArray) {
    ...
    guard let token = Property(rawValue: properties[0].key)  else {
      ...
      log("Wan Listener: unknown application property, \(properties[1].key)", .warning, #function, #file, #line)
      return
    }
    switch token {
  ...
  private func parseRadio(_ properties: KeyValuesArray, msg: String) {
    ...
    guard let token = Property(rawValue: properties[0].key)  else {
      // log it and ignore the message
      log("Wan Listener: unknown radio property, \(properties[1].key)", .warning, #function, #file,#line)
      return
    }
  ...
  private func parseApplicationInfo(_ properties: KeyValuesArray) {
    ...
    log("Wan Listener: ApplicationInfo received", .debug, #function, #file, #line)
    ...
    for property in properties {
      ...
      guard let token = Property(rawValue: property.key)  else {
        ...
        log("Wan Listener: unknown info property, \(property.key)", .warning, #function, #file, #line)
        continue
      }
  ...
  private func parseRegistrationInvalid(_ properties: KeyValuesArray) {
        log("Wan Listener: invalid registration: \(properties.count == 3 ? properties[2].key : "")", .warning, #function, #file, #line)
  }
  ...
  private func parseUserSettings(_ properties: KeyValuesArray) {
    ...
    log("Wan Listener: UserSettings received", .debug, #function, #file, #line)
    ...
    for property in properties {
      ...
      guard let token = Property(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Listener: unknown user setting, \(property.key)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
  ...
  private func parseRadioConnectReady(_ properties: KeyValuesArray) {
    ...
    log("Wan Listener: ConnectReady received", .debug, #function, #file, #line)
    ...
    for property in properties {
      ...
      guard let token = Property(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Listener: unknown connect property, \(property.key)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
    ...
    if _wanHandle != nil && _serial != nil {
      activeContinuation?.resume(returning: _wanHandle!)
    } else {
      activeContinuation?.resume(throwing: ListenerError.wanConnect)
    }
  }
  ...
  private func parseRadioList(_ msg: String.SubSequence) {
    ...
    log("Wan Listener: RadioList RECEIVED", .debug, #function, #file, #line)
...
//      }
  ...
  private func parseTestConnectionResults(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = Property(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Listener: unknown testConnection property, \(property.key)", .warning, #function, #file, #line)
        continue
      }
